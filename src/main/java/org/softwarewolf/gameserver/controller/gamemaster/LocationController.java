...
public class LocationController {
	...
	public String postEditLocation(HttpSession session, final LocationCreator locationCreator, 
		...
		try {
			...
			if (locationCreator.getCreateLocationTag()) {
				SimpleTag locationTag = new SimpleTag(location.getName(), campaignId);
				simpleTagService.save(locationTag);
			}
			if (locationCreator.getCreateLocationTypeTag()) {
				String gameDataTypeId = location.getGameDataTypeId();
				if (gameDataTypeId != null) {
					LocationType locationType = locationTypeService.findOne(gameDataTypeId);
					if (locationType != null && locationType.getName() != null) {
						SimpleTag locationTypeTag = new SimpleTag(locationType.getName(), campaignId);
